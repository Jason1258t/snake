...
public:
    ...
    Vector2D generatePosition(const GameField& field, const std::deque<Vector2D>& excludePositions) {
        ...
        while (!positionValid) {
            ...
            positionValid = isPositionValid(candidate, excludePositions) && 
                           field.isPositionEmpty(candidate);
        }
...
private:
    ...
    static Vector2D getDirectionFromInput(char input) {
        switch (input) {
            case 'w': case 'W': case 72: return Direction::UP;
            case 's': case 'S': case 80: return Direction::DOWN;
            case 'a': case 'A': case 75: return Direction::LEFT;
            case 'd': case 'D': case 77: return Direction::RIGHT;
...
public:
    explicit Game(IRenderer* renderInstance) : renderer(renderInstance), running(true) {}
