...
namespace InputManager
{
	InputEvent InputManager::MapKeyToEvent(int keyCode)
	{
		...
		case 'W':
		case KEY_UP:
			return InputEvent::DIRECTION_UP;

		case 's':
		case 'S':
		case KEY_DOWN:
			return InputEvent::DIRECTION_DOWN;

		case 'a':
		case 'A':
		case KEY_LEFT:
			return InputEvent::DIRECTION_LEFT;

		case 'd':
		case 'D':
		case KEY_RIGHT:
			return InputEvent::DIRECTION_RIGHT;

		case 'p':
		case 'P':
		case 27:
			return InputEvent::PAUSE_TOGGLE;
		case ' ':
			return InputEvent::PAUSE_TOGGLE;
		}
	...
	void InputManager::SetOnDirection(DirectionCallback callback)
	{
		directionCallback = callback;
	}

	void InputManager::SetOnPause(ActionCallback callback)
	{
	...
	void InputManager::ProcessEvent(InputEvent& event)
	{
		if (event >= InputEvent::DIRECTION_UP && event <= InputEvent::DIRECTION_RIGHT)
		{
			if (directionCallback)
			{
				...
				switch (event)
				{
				case InputEvent::DIRECTION_UP:
					direction = Direction::UP;
					break;
				case InputEvent::DIRECTION_DOWN:
					direction = Direction::DOWN;
					break;
				case InputEvent::DIRECTION_LEFT:
					direction = Direction::LEFT;
					break;
				case InputEvent::DIRECTION_RIGHT:
